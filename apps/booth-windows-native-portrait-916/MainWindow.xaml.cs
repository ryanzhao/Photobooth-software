...
public partial class MainWindow : Window
{
    ...
    private const int LivePreviewDecodePixelWidth = 960;
    ...
    private string? _guideOverlayCacheKey;
    ...
    {
        if (_selectedSourceMode == NativeSourceMode.Camera && _lastRuntime is not null)
        {
    ...
    private async Task UpdateLivePreviewAsync(BoothRuntimeStatus runtime, bool forceRefresh = false)
    {
        ...
        try
        {
            ...
            bitmap.DecodePixelWidth = LivePreviewDecodePixelWidth;
    ...
    private void UpdateLiveGuideOverlay()
    {
        ...
        var photos = GetCurrentSessionPhotos(session?.Id).OrderBy(photo => photo.SlotIndex).ThenBy(photo => photo.CapturedAt).ToList();
        var photoSignature = string.Join(",", photos.Select(photo => $"{photo.Id}:{photo.SlotIndex}:{photo.CapturedAt:O}:{photo.EditScale:F3}:{photo.EditRotation:F2}:{photo.EditOffsetX:F1}:{photo.EditOffsetY:F1}"));
        var cacheKey = $"{template.Id}|{_selectedFrameId}|{targetWidth}x{targetHeight}|{photoSignature}";
        if (string.Equals(cacheKey, _guideOverlayCacheKey, StringComparison.Ordinal) && LiveGuideOverlayImage.Source is not null)
        {
        ...
        LiveGuideOverlayImage.Visibility = Visibility.Visible;
        _guideOverlayCacheKey = cacheKey;
    }
